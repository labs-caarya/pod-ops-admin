      ...
      {members.map((m) => (
        <Card key={m.id} hover className="flex flex-col p-4">
          <div className="flex items-start gap-3">
            <Avatar name={m.name} color={m.avatarColor} className="h-11 w-11 text-sm" />
            <button onClick={() => onOpen(m)} className="min-w-0 flex-1 text-left">
              <p className="truncate font-display font-bold text-ink">{m.name}</p>
              <p className="truncate text-xs text-ink-muted">{m.primarySkill} · {m.level}</p>
            </button>
            <Badge tone={TALENT_STATUS_TONE[m.status] ?? "muted"}>{m.status}</Badge>
          </div>
          ...
          <div className="mt-3 space-y-1 text-xs text-ink-faint">
            <p className="flex items-center gap-1.5"><MapPin className="h-3 w-3" /> {m.college}</p>
            <p className="flex items-center gap-1.5"><Briefcase className="h-3 w-3" /> {m.availability}{m.placedAt ? ` · ${m.placedAt}` : ""}</p>
          ...
          <div className="mt-4 flex items-center gap-2 border-t border-line pt-3">
            ...
            <Button
              size="sm"
              variant={m.status === "Placed" ? "outline" : "primary"}
              className="flex-1"
              onClick={() => togglePlace(m)}
            >
              {m.status === "Placed" ? "Unplace" : "Place"}
            </Button>
          </div>
        </Card>
      ))}
